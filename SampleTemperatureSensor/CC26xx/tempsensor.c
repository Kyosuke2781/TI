...
static uint16 iCounter = 0;
typedef struct {
  uint16 Tag;
  uint8 iPH;
  uint16 iTDS;
  uint16 iTemp;
  uint16 iSrcShortAddr;
  //uint32_t iTimeStamp;
  //uint16_t iTimeStamp;
} sSensorData;
...
void TempSensor_task(NVINTF_nvFuncts_t *pfnNV)
{
    ...
    // Start EZMode Commissioning Mine
    ...
    for(;;)
    {
        ...
        if(ICall_wait(ICALL_TIMEOUT_FOREVER) == ICALL_ERRNO_SUCCESS)
        {
            ...
            if(pNwkInfo)
            {
              ...
              //MyADC();
              DataSend();
              Task_sleep (1000000/ Clock_tickPeriod); //(600000000/ Clock_tickPeriod); 10 Minute // Might cause and issue with polling
            }
...
static void DataSend( void )
{
  ...
  uint8 sData[20];
  //Types_freqHz *getFreq;
  //unsigned char *cpTempBuf;
  
  //uint32_t sec;
  //uint32_t timeNow = 1494811426;
  //Seconds_set(timeNow);
  
  sSensorData sLclSensorData;
  zclReportCmd_t *pReportCmd;
  
  //sec = Seconds_get();
  //sec = AONRTCSecGet();
  //sec = AONRTCFractionGet();
  
  //sec = TimestampProvider_get32();
  //TimestampProvider_getFreq(getFreq);
  ...
  {
#endif //__OK_SEND__
      sLclSensorData.Tag = 65020;
      sLclSensorData.iPH = 5;    //MyADC(ADC_COMPB_IN_AUXIO7);
      sLclSensorData.iTDS = 145; //MyADC(ADC_COMPB_IN_AUXIO7); // Cahaya
      sLclSensorData.iTemp = 15; //MyADC(ADC_COMPB_IN_AUXIO5); // Tanah
      sLclSensorData.iSrcShortAddr = pNwkInfo->nwkAddr;
      //sLclSensorData.iTimeStamp = sec;
  
      Util_uint16toa(sLclSensorData.Tag, (char *)&sData[0] );
      Util_itoa(sLclSensorData.iPH, &sData[4],10);
      Util_uint16toa(sLclSensorData.iTDS, (char *)&sData[6] );
      Util_uint16toa(sLclSensorData.iTemp, (char *)&sData[10] );
      Util_uint16toa(sLclSensorData.iSrcShortAddr, (char *)&sData[14] );
      //cpTempBuf = Util_ltoa(sLclSensorData.iTimeStamp, &sData[16],10 );
      //Util_uint16toa(sLclSensorData.iTimeStamp, (char *)&sData[16] );
      ...
      afDstAddr.endPoint = 0x07;//0xff;
      
 #ifndef __OK_SEND__
      // Fill in the single attribute information for the temperature reading
      pReportCmd->numAttr = 3;
      pReportCmd->attrList[0].attrID = ATTRID_MS_TEMPERATURE_MEASURED_VALUE;
      pReportCmd->attrList[0].dataType = ZCL_DATATYPE_INT16;
      pReportCmd->attrList[0].attrData = (void *)(&sLclSensorData.iTemp);
      
      pReportCmd->attrList[1].attrID = ATTRID_MS_TEMPERATURE_MIN_MEASURED_VALUE;
      pReportCmd->attrList[1].dataType = ZCL_DATATYPE_INT16;
      pReportCmd->attrList[1].attrData = (void *)(&sLclSensorData.iTDS);
      
      pReportCmd->attrList[2].attrID = ATTRID_MS_TEMPERATURE_MAX_MEASURED_VALUE;
      pReportCmd->attrList[2].dataType = ZCL_DATATYPE_INT16;
      pReportCmd->attrList[3].attrData = (void *)(&sLclSensorData.iPH);
        // Call ZCL function to send the report
       
      zcl_SendReportCmd(TEMPSENSOR_EP, &afDstAddr,
                          ZCL_CLUSTER_ID_MS_TEMPERATURE_MEASUREMENT,
                          pReportCmd,
                          ZCL_FRAME_SERVER_CLIENT_DIR,
                          true,
  ...
  zcl_SendCommand( ezModeData.endpoint, &afDstAddr, ZCL_CLUSTER_ID_GEN_IDENTIFY, COMMAND_IDENTIFY_QUERY, TRUE, ZCL_FRAME_CLIENT_SERVER_DIR, TRUE, 0, iCounter++, 20, &sData[0]);
  //zcl_SendCommand( ezModeData.endpoint, &afDstAddr, ZCL_CLUSTER_ID_MS_TEMPERATURE_MEASUREMENT, COMMAND_IDENTIFY_QUERY, TRUE, ZCL_FRAME_CLIENT_SERVER_DIR, TRUE, 0, iCounter++,20, &sData[0] );
...
int_fast16_t MyADC (uint_fast8_t iSource)
{
    uint16_t     i;
    //ADC_Handle   adc;
    //ADC_Params   params;
    ...
    AUXADCSelectInput(iSource);//ADC_COMPB_IN_AUXIO5);

    // Set up ADC
    AUXADCEnableSync(ADCCC26XX_FIXED_REFERENCE, ADCCC26XX_SAMPLING_DURATION_2P7_US, ADCCC26XX_TRIGGER_MANUAL);
...
*/
    ...
    for (i = 0; i < ADC_SAMPLE_COUNT; i++) {
        ...
        adcValue1[i] = AUXADCReadFifo();
        iValue = iValue + adcValue1[i];
 /*
 ...
 */
    ...
    iValue = iValue/ADC_SAMPLE_COUNT;

    //Util_uint16toa(adcValue1[i], (char *)&sData[0] );
    Util_uint16toa(iValue, (char *)&sData[0] );
    //Mine 
    //LCD_WRITE_STRING((char *)sData,  LCD_PAGE7);
